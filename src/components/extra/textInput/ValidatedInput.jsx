...
const ValidatedInput = ({
  ...
  BoxShadow,
  iconFontSize,
  onValidate, // This prop will be used to notify the parent component about the validation status
}) => {
  const [value, setValue] = useState("");
  const [touched, setTouched] = useState(false);
  const [error, setError] = useState("");
  ...
  const handleChange = (e) => {
    ...
    const isValid = appliedValidate(inputValue);
    if (isValid) {
      setError("");
      setShowRequiredError(false);
    } else {
    ...
    if (onValidate) {
      onValidate(isValid); // Notify parent about the current validity status
    }
  };
  ...
  const handleBlur = () => {
    ...
    if (!value) {
      ...
      setShowRequiredError(true);
      if (onValidate) {
        onValidate(false); // Field is not valid
      }
    } else if (!appliedValidate(value)) {
      setError(errorMessage);
      setShowRequiredError(false);
      ...
      setShowRequiredError(false);
      if (onValidate) {
        onValidate(true); // Field is valid
      }
    }
  };
